...
- **Everything visual is on placeholder gradients** — no real imagery exists (client supplied only screenshots). `Placeholder.astro` holds each slot and names its ASSETS.md entry; `document.querySelectorAll('[data-asset]')` lists the **25 outstanding images**. Generation list + priority: ASSETS.md.
...
3. **Phase 4 — remaining animations:** preloader (seal stamp), stat count-ups, map dot pulses + SVG arc draws (`stroke-dashoffset`, hooks already in the markup), process line draw, journal image parallax, page transitions (Astro view transitions). The mobile menu is already done.
