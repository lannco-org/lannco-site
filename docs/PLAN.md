...
- 2026-07-29 (session 2) — **Vercel project created:** `lannco-site`, scope `ryans-projects-ecaa36e6` (personal account of `r4hc-v`), project id `prj_lIyZpoyPKyRtrF7KG3XnDaQ2MqLn`. Local dir linked via `vercel link` (`.vercel/`, gitignored). **Nothing deployed yet** — no preview, no production. Two things still open before Phase 7 launch: (a) confirm the account is on **Pro**, since it's a personal scope and Hobby prohibits client work; (b) no git repo exists, so there are no auto-deploys on commit — CLI deploys only until a GitHub repo is created and connected.
